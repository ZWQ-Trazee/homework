...
export default function App1() {
	...
	return (
		<>
      请输入一个数字
			<input onChange={(e) => { setInputnum(Number(e.target.value)) }} />
			<button
				type="button"
				style={obj}
				onClick={() => { compare(inputnum, guessnum) }}
			>
        对比

			</button>
			<button type="button" style={obj} onClick={() => { setGuessnum(init()); setOutput('') }}>重置</button>
